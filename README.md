...
Use the exact address you use to access your panel in a browser:
...
```python
token = xui.create_api_token("new-token")
print(token["token"])  # shown only once 
```
...
xui.create_api_token("new-token")          # mint a new one — it's shown once
